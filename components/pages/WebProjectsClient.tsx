...
interface WebProject {
  ...
  tags: string[];
  href: string;
  /** Playable build on the shared Game Engine. Games only - the slug is the
   *  engine's own route id, which is not always the portfolio's (Aether
   *  Dynasty ships as matching-game, PropBet as sky-crash). */
  play?: string;
}

const ENGINE = 'https://game-engine-snowy.vercel.app/games';
          ...
          {WEB_PROJECTS.map((c) => (
            <div key={c.title} className="project-card type-web" data-category={c.cat} style={cssVars({ '--card-index': c.idx })}>
              ...
              <div className="project-links">
                ...
                {c.play ? (
                  <a href={c.play} target="_blank" rel="noopener" className="project-link-btn">Play the Game ↗</a>
                ) : null}
              </div>
            </div>
          ))}
