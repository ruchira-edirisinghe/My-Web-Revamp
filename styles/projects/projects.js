(function () {
    ...
    function drawFrame(ts) {
      ...
      if (raw < 1) requestAnimationFrame(drawFrame);
        ...
        setTimeout(() => {
          ...
          requestAnimationFrame(() => { splitTop.classList.add('open'); splitBottom.classList.add('open'); });
          setTimeout(() => { preloader.remove(); splitTop.remove(); splitBottom.remove(); }, SPLIT_MS + 100);
        }, HOLD_MS);
      }
    }
  ...
  (function () {
    ...
    document.querySelectorAll('a, button, .social-card, .platform-card, .stream-card, .project-card').forEach(el => {
      el.addEventListener('mouseenter', () => { targetR = R_HOVER; playHover(); });
      el.addEventListener('mouseleave', () => { targetR = R_NORMAL; });
      el.addEventListener('click', () => { spawnParticles(mX, mY); playClick(); });
    });
  ...
  (function () {
    ...
    window.addEventListener('beforeunload', () => { sessionStorage.setItem('musicTime', String(bgAudio.currentTime)); sessionStorage.setItem('musicMuted', String(musicMuted)); });
    ['click', 'mousemove', 'keydown'].forEach(ev => window.addEventListener(ev, startMusic, { once: true }));
  ...
  (function () {
    ...
    document.querySelectorAll('.sr-elem, .social-card, .stream-card, .project-card').forEach(el => obs.observe(el));
  })();
